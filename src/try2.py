...
def vectorize_output(data, word_idx):
    y = np.zeros(shape= (len(word_idx) + 1),dtype='uint8')
    ys = []
    for story, query, answer in data:
        for w in answer:
...
#function for splitting into batches#
def batches(data,batch_idx,word_idx,story_maxlen,query_maxlen,batch_size = BATCH_SIZE):
    ...
    y = vectorize_output(train[batch_idx:batch_idx+batch_size],word_idx)
    return x,xq,y
...
#question and paragraph separately #
sentence = layers.Input(shape=(story_maxlen,), dtype='int32')
encoded_sentence = layers.Embedding(vocab_size, EMBED_HIDDEN_SIZE)(sentence)
#test with and without the LSTM to see intuitive difference in performance.
...
#merge both the layers to learn how question is encoded in the sentence#
...
#####
output=model.predict([batch_X,batch_XQ])
print(output)
#####
loss, acc = model.evaluate([batch_X,batch_XQ], batch_y,batch_size=BATCH_SIZE)
